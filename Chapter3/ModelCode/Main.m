...
inp_pathname = '/rdsi/vol08/homes/jc442626/InputFiles_C3';
op_pathname = '/rdsi/vol08/homes/jc442626/OutputFiles_NullModel_C3/';



for tf = 1 : 50 % loop for the shape files 
...
%             %the right edge Ignore right/neighbour on the EAST which is the first elelement of neigh_array
%             neigh_array_eight(i,1) = -1;
%         end
%         if (mod(i-1,m_offset)==0)%Ignore left/neighbour on the WEST which is the first elelement of neigh_array
%             neigh_array_eight(i,2) = -1;
%         end
%         if ((i+m_offset)>no_cell)%Ignore the upper/north if it is more than no_cell
%             neigh_array_eight(i,3)= -1;
%         end
%         if ((i+m_offset-1)>no_cell) %NW
%             neigh_array_eight(i,7)= -1;
%         end
%         if ((i+m_offset+1)>no_cell) %NW
%             neigh_array_eight(i,8)= -1;
%         end
%     end
...
%       end
    ...
    if ((z>4) && (rem(z,5)==0))  %Update LUZ at every 5th iteration, check for 10th iteration  
        ...
        elseif (updaterules == 2)
            [newS] = Regional_CA(nextS,mkfolder,z,no_cell); 
        else
